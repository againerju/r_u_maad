...
from src.utils import gpu, to_long
from src.layers import Conv1d, Res1d, Linear, LinearRes


class LaneGCNAE(nn.Module):
    """
    ...
    def forward(self, data: Dict, output_latent=False) -> Dict[str, List[Tensor]]:
...
class MapNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        for i in range(self.config.model.num_scales):
...
class A2M(nn.Module):
    ...
    def __init__(self, config):
        ...
        att = []
        for i in range(2):
            att.append(Att(n_map, self.config.model.n_actor))
    ...
    def forward(self, feat: Tensor, graph: Dict[str, Union[List[Tensor], Tensor, List[Dict[str, Tensor]], Dict[str, Tensor]]], actors: Tensor, actor_idcs: List[Tensor], actor_ctrs: List[Tensor]) -> Tensor:
        ...
        for i in range(len(self.att)):
            feat = self.att[i](
                ...
                actor_idcs,
                actor_ctrs,
                self.config.model.actor2map_dist,
            )
        return feat
...
class M2A(nn.Module):
    ...
    def __init__(self, config):
        ...
        n_actor = self.config.model.n_actor
        n_map = self.config.model.n_map
    ...
    def forward(self, actors: Tensor, actor_idcs: List[Tensor], actor_ctrs: List[Tensor]) -> Tensor:
        for i in range(len(self.att)):
            actors = self.att[i](
                ...
                actor_idcs,
                actor_ctrs,
                self.config.model.actor2actor_dist,
            )
        return actors
...
class PredNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        pred = []
        for i in range(self.config.model.num_mods):
            pred.append(
                nn.Sequential(
                    LinearRes(n_actor, n_actor, norm=norm, ng=ng),
                    nn.Linear(n_actor, 2 * self.config.model.num_preds),
                )
            )
    ...
    def forward(self, actors: Tensor, actor_idcs: List[Tensor], actor_ctrs: List[Tensor]) -> Dict[str, List[Tensor]]:
        ...
        for i in range(len(actor_idcs)):
            ...
            out["latent_features"].append(actors[idcs])
        return out
